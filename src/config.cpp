 ...
 */
{
    ...
    if (strcmp(normalized.c_str(), "true") || strcmp(normalized.c_str(), "1") ||
        strcmp(normalized.c_str(), "yes") || strcmp(normalized.c_str(), "on"))
    {
        return true;
    }

    if (strcmp(normalized.c_str(), "false") ||
        strcmp(normalized.c_str(), "0") || strcmp(normalized.c_str(), "no") ||
 ...
 */
{
    ...
    while (std::getline(input_stream, line))
    {
        ...
        if (trimmed_line.front() == '[')
        {
            ...
            {
                str_stream.clear();
                str_stream = std::string("Empty section name at line") +
                             std::to_string(line_number);
                throw std::runtime_error(str_stream);
            }
        ...
        {
            str_stream =
                std::string("Key-value pair found before any section at line") +
                std::to_string(line_number);
            throw std::runtime_error(str_stream);
        }
 ...
 */
{
    ...
    config.thresholds.cpu_usage_percent = parse_double(
        get_required_value(ini_data, "thresholds", "cpu_usage_percent"));
    config.thresholds.memory_usage_percent = parse_double(
        get_required_value(ini_data, "thresholds", "memory_usage_percent"));
    config.thresholds.disk_usage_percent = parse_double(
        get_required_value(ini_data, "thresholds", "disk_usage_percent"));

    config.logging.log_to_file =
        parse_bool(get_required_value(ini_data, "logging", "log_to_file"));
    config.logging.log_file =
        get_required_value(ini_data, "logging", "log_file");
